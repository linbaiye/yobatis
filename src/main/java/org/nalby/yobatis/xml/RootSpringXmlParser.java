...
public class RootSpringXmlParser extends BasicXmlParser {
	...
	private static final String[] DATASOURCE_CLASSES = {"com.alibaba.druid.pool.DruidDataSource",
	...
	private boolean isDatasourceBean(Element element) {
		...
		return DATASOURCE_CLASSES[0].equals(clazz) || DATASOURCE_CLASSES[1].equals(clazz);
	}
	...
	/**
	 * Get the driver class' name from the datasource bean. if there are multiple
	 * datasource beans, the first one will be used.
