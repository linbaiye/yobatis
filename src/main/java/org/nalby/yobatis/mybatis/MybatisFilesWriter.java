...
public class MybatisFilesWriter {
	...
	private String mergeManualSqlMapper(String path, String content) {
		...
		try {
			inputStream = project.getInputStream(manualPath);
			SqlMapperParser manualParser = new SqlMapperParser(inputStream);
			SqlMapperParser autoParser = SqlMapperParser.fromString(content);
			autoParser.merge(manualParser);
			return autoParser.toXmlString();
		} catch (Exception e) {
			return content;
		} finally {
